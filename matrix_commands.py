...
def validate_control_room(bot, config):
    room = config.matrix_console_room_id
    ...
    if bot.user_id == config.matrix_control_user:
    ...
    expected = {bot.user_id, config.matrix_control_user}
    ...
    controller_level = users.get(config.matrix_control_user, default_user)
    ...
    try:
        ...
        rooms = mapping.get(config.matrix_control_user) if isinstance(mapping, dict) else None
...
def command_from_event(event, config):
    if (not isinstance(event, dict) or event.get('type') != 'm.room.message'
            or event.get('sender') != config.matrix_control_user):
        return None
...
class MatrixCommandListener:
    ...
    def _events_from_sync(self, response):
        ...
        room = joined.get(self.config.matrix_console_room_id, {}) if isinstance(joined, dict) else {}
        ...
        return response['next_batch'], [command for event in events
                                        if (command := command_from_event(event, self.config)) is not None]

    def bootstrap(self):
        validate_control_room(self.bot, self.config)
        if self.state.snapshot()['since'] is not None:
            return False
        response = self.bot.sync(since=None, room_id=self.config.matrix_console_room_id, timeout_ms=0)
    ...
    def _plan_queued(self, job):
        ...
        if command in ('KI an', 'KI aus'):
            ...
            if enabled:
                try:
                    ...
                    self.state.set_ai(event_id, False)
                    parts = build_failure_ack(self.config, event_id, 'ai_unavailable')
                    self.state.plan_head(event_id, False, parts)
                    return
            self.state.set_ai(event_id, enabled)
        ...
        try:
            ...
            parts = build_failure_ack(self.config, event_id, 'test_failed')
    ...
    def poll_once(self, *, timeout_ms=30_000):
        ...
        response = self.bot.sync(since=state['since'], room_id=self.config.matrix_console_room_id,
